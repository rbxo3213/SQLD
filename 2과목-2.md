...
- 메인쿼리의 컬럼이 포함되지 않은 서브쿼리를 '비연관 서브쿼리' 라고 한다.
...
- 비연관 서브쿼리: 메인쿼리와 관계X --> 서브쿼리 내에 메인쿼리의 컬럼이 존재하지 않음.
- 연관 서브쿼리: 메인쿼리와 관계O ----> 서브쿼리 내에 메인쿼리의 컬럼이 존재함.
...
ROW BETWEEN CURRENT ROW AND 5 FOLLOWING (현재 행부터 아래로 5 만큼 이동한 행 까지)
```
...
- LEAD, LAG는 행의 상대적 위치(앞/뒤 몇 번째 행)를 기준으로 동작
